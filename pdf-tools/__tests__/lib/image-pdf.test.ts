import { describe, it, expect } from "vitest";
import { PDFDocument } from "pdf-lib";
import { imagesToPdf } from "@/lib/pdf/jpg-to-pdf";
import { pdfToImages } from "@/lib/pdf/pdf-to-jpg";

function createMinimalJpeg(): Uint8Array {
  return new Uint8Array([
    ,
  ]);
}

function makeJpegFile(name = "test.jpg"): File {
  const data = createMinimalJpeg();
  const buffer = data.buffer.slice(0) as ArrayBuffer;
  return new File([buffer], name, { type: "image/jpeg" });
}

describe("imagesToPdf", () => {
  it("converts a single JPEG to a 1-page PDF", async () => {
    const file = makeJpegFile();
    const pdfBytes = await imagesToPdf([file]);

    expect(pdfBytes).toBeInstanceOf(Uint8Array);
    expect(pdfBytes.length).toBeGreaterThan(0);

    const pdfDoc = await PDFDocument.load(pdfBytes);
    expect(pdfDoc.getPageCount()).toBe(1);
  });

  it("converts multiple images to a multi-page PDF", async () => {
    const files = [makeJpegFile("a.jpg"), makeJpegFile("b.jpg"), makeJpegFile("c.jpg")];
    const pdfBytes = await imagesToPdf(files);

    const pdfDoc = await PDFDocument.load(pdfBytes);
    expect(pdfDoc.getPageCount()).toBe(3);
  });
});

describe("pdfToImages", () => {
  it("exports the function with correct signature", () => {
    expect(typeof pdfToImages).toBe("function");
    expect(pdfToImages.length).toBeGreaterThanOrEqual(1);
  });

  it.skip("renders PDF pages to image blobs (requires DOM/canvas)", async () => {
    const pdfBytes = await imagesToPdf([makeJpegFile()]);
    const buffer = pdfBytes.buffer.slice(0) as ArrayBuffer;
    const file = new File([buffer], "test.pdf", { type: "application/pdf" });
    const blobs = await pdfToImages(file);
    expect(blobs.length).toBe(1);
    expect(blobs[0]).toBeInstanceOf(Blob);
  });
});
