...
export default function DeletePagesPage() {
  ...
  const handleProcess = useCallback(async () => {
    ...
    startProcessing();
    try {
      updateProgress(40, "Deleting pages...");
      const buffer = await files[0].file.arrayBuffer();
      const resultBytes = await deletePages(new Uint8Array(buffer), Array.from(selectedPages));
      const blob = pdfBlob(resultBytes);
      completeProcessing(blob);
    } catch (e) {
      failProcessing(e instanceof Error ? e.message : "Failed to delete pages");
    }
  ...
  return (
    ...
    >
      ...
      <ProcessingView state={state} result={result} filename="pages-deleted.pdf" onReset={handleReset} />
    </ToolLayout>
  );
}
